...
## 读取数据集
rawtable <- read.csv("table/20180420-spike3-group1-3.csv", header = T)
colnames(rawtable)[1] = "bacteria"
table <- gather(rawtable, key = sample , value = count, `sample01`:`sample05`)

## 设置横轴变量为离散型因子
aatable4 <- table
aatable4$bacteria <- factor(table$bacteria)
aatable4$group <- factor(table$group)

## 生成箱线图
aaboxplot4 <- ggplot(aatable4) +
...
ggsave(paste("figure/20180420-spike3-group1-3.pdf", sep=""), aaboxplot4, width = 9, height = 5)
