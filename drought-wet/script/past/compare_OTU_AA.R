...
# 3. 读取输入文件
write.table("ID\ttype", file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare//diff.list", sep=""), append = F, sep="\t", quote=F, row.names=F, col.names=F)
...
# 读取OTU表
# otutab = read.table(paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/otutab.txt", sep=""), header=T, row.names=1, sep="\t", comment.char="") 
otutab = read.table(paste("~/xiaoxuan/180528/180627_AQ/bac/spikein/otutab_norm_AA.txt", sep=""), header=T, row.names=1, sep="\t", comment.char="")
# 显示OTU问题
...
# 按丰度按组中位数筛选
filtered = mat_mean_final[apply(mat_mean_final,1,max) >=0.005, ] # select OTU at least one sample > 0.1%
otutab = otutab[rownames(filtered),]

# 生成compare的database用于注释
mat_mean_high = mat_mean_final[rownames(filtered),]
write.table(paste("OTUID\t",sep=""), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", "database.txt",sep=""), append = F, quote = F, eol = "", row.names = F, col.names = F)
suppressWarnings(write.table(round(mat_mean_high*100,3), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", "database.txt",sep=""), append = T, quote = F, sep = '\t', row.names = T))
...
compare_DA = function(compare){
  ...
  # compare<- compare_data[1,]
	...
	if (FALSE){
		...
		sub_norm = as.matrix(otutab) * 10000+ 0.001 # 数据类型一致，计算后矩阵
	}
	# 建立两组的矩阵
	...
	write.table(paste(group_list[1],"_", group_list[2], "\t",sep=""), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", SampAvsB, "_all.txt",sep=""), append = F, quote = F, eol = "", row.names = F, col.names = F)
	suppressWarnings(write.table(output,file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", SampAvsB, "_all.txt",sep=""), append = T, quote = F, sep = '\t', row.names = T))

	# 计算上、下调OTUs数量，写入统计文件
	NoE= dim(output[output$level=="Enriched",])[1]
	NoD= dim(output[output$level=="Depleted",])[1]
	NoN= dim(output[output$level=="NotSig",])[1]
	suppressWarnings(write.table(paste( SampAvsB, NoE, NoD, NoN, sep="\t"), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", "summary.txt",sep=""), append = T, quote = F, sep = '\t', row.names = F, col.names = F))

	output=output[output$level!="NotSig",]
	# 保存筛选结果于sig.txt结尾文件中
	write.table(paste(group_list[1],"_", group_list[2], "\t",sep=""), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", SampAvsB, "_sig.txt",sep=""), append = F, quote = F, eol = "", row.names = F, col.names = F)
	suppressWarnings(write.table(output, file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", SampAvsB, "_sig.txt",sep=""), append = T, quote = F, sep = '\t', row.names = T))
	
	# 确保有差异才写入，否则会出不完整行
	if (dim(output)[1]>1){
	# 保存差异列表用于维恩图展示 save each group DA OTU list for venndiagram
	# 使用edgeR中的比较-相连，无法作为变量赋值，改为_
	write.table(cbind(rownames(output),paste(group_list[1],"_", group_list[2], output$level, sep="")), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare//diff.list", sep=""), append = TRUE, sep="\t", quote=F, row.names=F, col.names=F)
	}
}


# 记录各组间上、下调数量
write.table("GroupAvsB\tEnriched\tDepleted\tNotSig\n", file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare/", "summary.txt",sep=""), append = F, quote = F, eol = "", row.names = F, col.names = F)
...
system("sed -i 's/Enriched/_E/;s/Depleted/_D/;' /mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/result/compare//diff.list")
