...
# 读取usearch alpha文件
alpha = read.table("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/index.txt", header=T, row.names=1, sep="\t", comment.char="") 
...
for(m in method){
	...
	write.table(paste(m, "\n\t", sep=""), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/",m,".txt",sep=""),append = F, quote = F, eol = "", row.names = F, col.names = F)
	# 保存统计结果，有waring正常
	suppressWarnings(write.table(Tukey_HSD_table, file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/",m,".txt",sep=""), append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = T, col.names = T))
...
# 输出原始数据，方便筛选 
if (FALSE){
	write.table(paste("SampleID\t", sep=""), file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/",m,"_raw.txt",sep=""), append = F, quote = F, eol = "", row.names = F, col.names = F)
	suppressWarnings(write.table(index[,c(m,"group")], file=paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/",m,"_raw.txt",sep=""), append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = T, col.names = T))
}
	...
	p
	ggsave(paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/", m, ".pdf", sep=""), p, width = 8, height = 5)
	ggsave(paste("/mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/", m, ".png", sep=""), p, width = 8, height = 5)
	# 提示工作完成
	print(paste("Output in /mnt/bai/xiaoning/xiaoxuan/180528/180627_AQ/bac/unoise/result/alpha/", m, ".txt/pdf finished.", sep = ""))
}
