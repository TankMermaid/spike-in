...
method = c("p","c","o","f","g","pc")
for(m in method){
	# 读取usearch tax文件
	tax_sample = read.table(paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, ".txt", sep=""), header=T, row.names=1, sep="\t", comment.char="") 

	# 按丰度降序排序
	mean_sort = tax_sample[(order(-rowSums(tax_sample))), ]
	mean_sort = as.data.frame(mean_sort)
	# 筛选高丰度的 10 类，其它归为低丰度Low abundance，可设置选择数量，即图例显示数量
	...
	# 保存变量备份，并输出至文件
	merge_tax=mean_sort
	write.table("Taxonomy\t\t", file=paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_sample.txt",sep=""),append = F, quote = F, eol = "", row.names = F, col.names = F)
	suppressWarnings(write.table(merge_tax, file=paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_sample.txt",sep=""), append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = T, col.names = T))
	...
	p
	# 保存pdf和png格式方便查看和编辑
	ggsave(paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_sample.pdf", sep=""), p, width = 8, height = 5)
	ggsave(paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_sample.png", sep=""), p, width = 8, height = 5)
	print(paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_sample.pdf/png/txt finished.", sep = ""))
	...
	write.table("Taxonomy\t", file=paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_group.txt",sep=""),append = F, quote = F, eol = "", row.names = F, col.names = F)
	suppressWarnings(write.table(mean_sort, file=paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_group.txt",sep=""), append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = T, col.names = T))
	...
	# 保存pdf和png格式方便查看和编辑
	ggsave(paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_group.pdf", sep=""), p, width = 8, height = 5)
	ggsave(paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_group.png", sep=""), p, width = 8, height = 5)
	print(paste("/mnt/bai/yongxin/other/guoxiaoxuan/180627_AQ/bac/result/tax/sum_", m, "_group.pdf/txt finished.", sep = ""))
