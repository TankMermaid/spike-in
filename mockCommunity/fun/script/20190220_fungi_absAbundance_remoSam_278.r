...
## This scripot is meant to draw the absolute abundance with SpikeIn and without SpikeIn 
...
# # ord1 <- match(rownames(bac_li), rownames(sub_table))    ## row reorder
# absAbundance<- absAbundance[,ord]   ## parrellel
write.table(absAbundance,file = "./table/20190724_fungi_absAbundance_remoSam_278_absAbundance.xls",sep = '\t',row.names = T)
...
pre_BA <- mean_B[c(11:15),]
pre_BA$x <- NULL

# colnames(pre_BA)[7] <- "ration_CA"

aft_BA <- mean_B[c(1:10),]
aft_BA$x <- NULL
...
write.table(mean_A,file = "./table/20190724_fungi_absAbundance_remoSam_278mean_A_111_and_CA_ratio_merge.xls",sep = '\t',row.names = T)
write.table(mean_A,file = "./table/20190724_fungi_absAbundance_remoSam_278mean_A_111_and_CA_ratio.xls",sep = '\t',row.names = T)
write.table(mean_C,file = "./table/20190724_fungi_absAbundance_remoSam_278mean_C_222_and_CB_ratio.xls",sep = '\t',row.names = T)
write.table(mean_B,file = "./table/20190724_fungi_absAbundance_remoSam_278mean_B_221.xls",sep = '\t',row.names = T)
...
######CA########
p <- ggplot(mean_A[ !mean_A$Group.2 %in% "E00",], aes(x=Group.3, y=ration_CA,shape=Group.3,group=Group.1,color=Group.3)) +
  geom_line() +
  facet_wrap(~Group.2)+
  geom_point(size=3, fill="white") +
  scale_colour_manual(values=as.character(c(c_red, c_dark_brown,c_black,c_dark_green))) +
...
ggsave("./figure/20190724_fungi_absAbundance_remoSam_278AA_ratio_CvsA_line_chart.pdf", p)
...
ggsave("./figure/20190724_fungi_absAbundance_remoSam_278AA_ratio_error_boxplot.pdf", p)
...
ggsave("./figure/20190724_fungi_absAbundance_remoSam_278aftAB_AA_ratio_error_boxplot.pdf", p)
...
RA_ar <- ra_error<- read.table("table/20190724_fungi_remoSam_278RA_ratio_error_merge.txt", header=T,sep="\t") 
...
ggsave("./figure/20190724_fungi_remoSam_278AA_ratio_vsRA_ratio_error_boxplot.pdf", p)

write.table(df_error,file = "./table/20190724_fungi_remoSam_278AAvsRA_ratio_error.txt",sep = '\t',row.names = F,quote = F)
...
ggsave("./figure/20190724_fungi_remoSam_278AA_ratio_vsRA_ratio_error_line—plot_merge.pdf", p)

write.table(ar_data,file = "./table/20190724_fungi_remoSam_278AAvsRA_ratio_error_col_merge.txt",sep = '\t',row.names = F,quote = F)


####CB###
p <- ggplot(mean_C[!mean_C$Group.2 %in% "E00",], aes(x=Group.3, y=ration_CB,shape=Group.3,group=Group.1,color=Group.3)) +
  geom_line() +
  facet_wrap(Group.1~Group.2)+
  geom_point(size=3, fill="white") +
  scale_colour_manual(values=as.character(c(c_red, c_dark_brown,c_black,c_dark_green))) +
...
ggsave("./figure/20190724_fungi_absAbundance_remoSam_278AA_ratio_CvsB_line_chart.pdf", p)
