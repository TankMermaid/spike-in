...
write.table(sub_table_nor,file = "~/xiaoxuan/180528/180706_fungal/RA/otutab_norm_RA.txt",sep = '\t',row.names = T,quote = F)
...
ggsave("../plot/microbiome_load_Hn.pdf", p)


p = ggplot(data_all[data_all$Site %in% "AnHui"& data_all$Genotype %in% c("MH63","WYJ7DEP.1")&!data_all$Row.names %in% c("Fun2WMH6302","Fun2WMH6307"),], aes(x=Condition, y = Microbiome_Load,color=Condition, fill=Condition)) + 
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.4, fill="transparent") +
  geom_jitter(aes(shape=Condition), position=position_jitter(0.17), size=1, alpha=0.7) +
  scale_colour_manual(values=as.character(colors$color)) +
  scale_shape_manual(values=shapes$shape) +
...
ggsave("../plot/microbiome_load_Ah.pdf", p)
...
write.table(bac_reads_AA_cor,file = "~/xiaoxuan/180528/180706_fungal/table/adjusted_absAbundance.txt",sep = '\t',row.names = T,quote = F)
write.table(bac_reads_AA_cor,file = "~/xiaoxuan/180528/180706_fungal/spikein/otutab_norm_AA.txt",sep = '\t',row.names = T,quote = F)




# write.table(bac_reads_AA_cor,file = "../table/adjusted_bac_reads_AA_needToChangeLabelOfMh63andZH.txt",sep = '\t',row.names = T,quote = F)
