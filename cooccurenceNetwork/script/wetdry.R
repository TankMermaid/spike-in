...
figures.dir <- paste("~/xiaoxuan/180528/fungal/plot/",tem,'/',sep = '')
table.dir <- paste("~/xiaoxuan/180528/fungal/table/",tem,'/',sep = '')
...
ggsave("../plot/phylum_RA_sample_NO1_allPhylum.pdf", p)
...
ggsave("../plot/phylum_RA_sample_NO2_allPhylum.pdf", p)


wilco_test_Phylunm_batchPlusgeno <- compare_means(value ~ Other , data_all, group.by = c("phylumpro","Bactch","Genotype"))
write.table(wilco_test_Phylunm_batchPlusgeno, file="../table/wetDryRice_wilcoxon_test/wilco_test_Phylunm_batchPlusgeno.txt", append = FALSE, sep="\t", quote=F, row.names=F, col.names=F)

mean_sort$phylumpro <- row.names(mean_sort)
wilco_test_Phylunm_batchPlusgeno_detail <- merge(wilco_test_Phylunm_batchPlusgeno,mean_sort,by = "phylumpro")
write.table(wilco_test_Phylunm_batchPlusgeno, file="../table/wetDryRice_wilcoxon_test/wilco_test_Phylunm_batchPlusgeno.txt", append = FALSE, sep="\t", quote=F, row.names=F, col.names=F)
write.table(wilco_test_Phylunm_batchPlusgeno_detail, file="../table/wetDryRice_wilcoxon_test/wilco_test_Phylunm_batchPlusgeno_detail.txt", append = FALSE, sep="\t", quote=F, row.names=F, col.names=F)


wilco_test_Phylunm_batch <- compare_means(value ~ Other , data_all, group.by = c("phylumpro","Bactch"))
write.table(wilco_test_Phylunm_batch, file="../table/wetDryRice_wilcoxon_test/wilco_test_Phylunm_batch.txt", append = FALSE, sep="\t", quote=F, row.names=F, col.names=F)
...
# 
# p <- ggplot(data_all[data_all$Bactch %in% "NO.1"&data_all$phylumpro %in% c("Chloroflexi","Tenericutes"),], aes(x=Other, y=value,color=Other, fill=Other)) +
#   geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
#   geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
#   labs(x="Growth Condition_batch_NO1", y="Relative phylum abundance") +
#   # geom_signif(comparisons =list(c("E05/5", "E00")),
...
# 
# p <- ggplot(data_all[data_all$Bactch %in% "NO.2"&data_all$phylumpro %in% c("Chloroflexi","Tenericutes"),], aes(x=Other, y=value,color=Other, fill=Other)) +
#   geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
#   geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
#   labs(x="Growth Condition_batch_NO2", y="Relative phylum abundance") +
#   
...
#   }
# }else {
#   compare_data = read.table("/mnt/bai/xiaoning/project/jingying/Unoise_NRT/doc/NRT/group_compare.txt", sep="\t", check.names=F, quote='', com='')
...
write.table(bac_reads_AA,file = "../table/absAbundance.xls",sep = '\t',row.names = T)


micro_load <- as.data.frame(t(colSums(bac_reads_AA)))
idp <- match(row.names(qpcr),colnames(micro_load))
qpcr_ratio<- qpcr[idp,]
...
p = ggplot(data_all[data_all$Bactch %in% "NO.1",], aes(x=Other, y = Microbiome_Load,color=Other, fill=Other)) + 
  ...
  # 关闭x轴刻度和标签
  # theme(axis.ticks.x = element_blank(), axis.text.x = element_blank())+
  xlab("Groups_Batch_NO.1")+ylab("Microbiome_Fungi Load (Total Reads / Spike-in)")+
  theme(axis.text.x = element_text(size=7.5,angle = 90))+
  stat_compare_means(method="wilcox.test", size=3)+
  # geom_signif(comparisons =list(c("dry", "wet")),
  #             # annotations = "No Sig.",
...
ggsave("../plot/microbiome_load_NO1.pdf", p)
...
p = ggplot(data_all[data_all$Bactch %in% "NO.2",], aes(x=Other, y = Microbiome_Load,color=Other, fill=Other)) + 
  ...
  # 关闭x轴刻度和标签
  # theme(axis.ticks.x = element_blank(), axis.text.x = element_blank())+
  xlab("Groups_Batch_NO.2")+ylab("Microbiome_Fungi Load (Total Reads / Spike-in)")+
  theme(axis.text.x = element_text(size=7.5,angle = 90))+
  stat_compare_means(method="wilcox.test", size=3)+
  # geom_signif(comparisons =list(c("dry", "wet")),
  #             # annotations = "No Sig.",
...
ggsave("../plot/microbiome_load_NO2.pdf", p)
...
idp <- match(row.names(qpcr),colnames(bac_reads_AA))
qpcr_ratio<- qpcr[idp,]
bac_reads_AA_cor <- sweep(bac_reads_AA, 2, qpcr_ratio$CopyNumber, "/")

write.table(bac_reads_AA_cor,file = "../table/adjusted_fungi_reads_AA.txt",sep = '\t',row.names = T,quote = F)
...
ggsave("../plot/AA_phylum_stack_phylumpro_sample_NO1_allPhylum.pdf", p)
...
p = ggplot(data_all[data_all$Bactch %in% "NO.2",], aes(x=variable, y = value, fill = phylumpro,group=variable)) + 
  ...
  main_theme
p
ggsave("../plot/AA_phylum_stack_phylumpro_sample_NO2_allPhylum.pdf", p)
...
p <- ggplot(points, aes(x=x, y=z, color=Genotype,shape=Bactch)) +
  geom_point(alpha=.7, size=5) +
  scale_colour_manual(values=as.character(colors$color)) +
  scale_shape_manual(values=shapes$shape) +
  labs(x=paste("PCoA 1 (", format(100 * eig[1] / sum(eig), digits=4), "%)", sep=""),
