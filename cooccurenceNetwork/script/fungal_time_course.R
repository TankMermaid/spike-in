...
figures.dir <- paste("~/xiaoxuan/180528/fungal/plot/lifeCycleRice/",tem,'/',sep = '')
table.dir <- paste("~/xiaoxuan/180528/fungal/table/lifeCycleRice/",tem,'/',sep = '')
...
design = read.table("../doc/fun_time_course_design.txt", header=T, row.names= 1, sep="\t") 
...
write.table(sub_table,file = "../table/lifeCycleRice/filtered_rarefy.txt",sep = '\t',row.names = T,quote = F)
...
write.table(otu_tax,file = "../table/lifeCycleRice/filtered_OTU_rarefy_tax.txt",sep = '\t',row.names = T,quote = F)
write.table(sub_OTU,file = "../table/lifeCycleRice/filtered_OTU_rarefy_ordered.txt",sep = '\t',row.names = T,quote = F)
...
batch_group <- unique(design$Batch)               # plasmid id 
# gra_group <- unique(design$Description)
design$Age <- factor(design$Age, levels = unique(design$Age)) ### implement
...
shapes <- data.frame(group=batch_group,
                     shape=c(19, 0))
...
detailName <- paste("Natural_LifeCycleRice_bacteria_Community_",spike[2],sep = '_')

p <- ggplot(index, aes(x=Age, y=value,color=Batch)) +
  facet_wrap(~Batch)+
  geom_boxplot(alpha=0.5, outlier.size=0, size=0.7, width=0.4, fill="transparent") +
  geom_jitter(aes(shape=Batch), position=position_jitter(0.17), size=1, alpha=0.7) +
...
ggsave("../plot/lifeCycleRice/filtered_spike_in_Abundance.pdf",  p)

write.table(index,file = "../table/lifeCycleRice/filtered_Facet_Natural_spikeAbun.txt",sep = '\t',row.names = F,quote = F)
...
write.table(paste("../table/lifeCycleRice/taxonomy\tSampAvsB\tPvalue",sep="\t"), file=paste("phylumpro.txt", sep=""), append = FALSE, sep="\t", quote=F, row.names=F, col.names=F)
...
p = ggplot(data_all, aes(x=variable, y = value, fill = phylumpro,group=Age)) + 
  geom_bar(stat = "identity")+ 
  # scale_y_continuous(labels = scales::percent) + 
  # 分面，进一步按group分组，x轴范围自由否则位置异常，swith设置标签底部，并调置去除图例背景
  facet_grid( ~ Batch, scales = "free_x", switch = "x") +  
...
ggsave("../plot/lifeCycleRice/tax_RA_sample_CPvsSZ.pdf", p)


# # ggsave("tax_stack_phylumpro_sample.png", p, width = 5, height = 3)
# p = ggplot(data_all, aes(x=variable, y = value, fill = phylumpro,group=variable)) + 
...
# p
# 
# ggsave("../plot/lifeCycleRice/tax_RA_sample_NO2.pdf", p)


write.table(data_all, file="../table/lifeCycleRice/tax_RA.topN", append = FALSE, sep="\t", quote=F, row.names=F, col.names=F)




# plot absolute abundance 
qpcr <- read.delim("../doc/time_course_qpcr.txt", row.names= 1,header=T, sep="\t",stringsAsFactors = F)
...
micro_load <- micro_load/ratio[2:123,1]
data_all = merge(micro_load, design, by = "row.names")
colnames(data_all)[2] <- "Microbiome_Load"
write.table(data_all,file = "../table/lifeCycleRice/bac_microbiome_load.txt",sep = '\t',row.names = F,quote = F)

p = ggplot(data_all[!data_all$Age %in% c(28,118),], aes(x=Age, y = Microbiome_Load,color=Batch, fill=Batch)) + 
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.4) +
  geom_jitter(position=position_jitter(0.17), size=1, alpha=0.7) +
  scale_colour_manual(values=c(c_red,c_black)) +
  # scale_shape_manual(values=shapes$shape) +
  # scale_y_continuous(labels = scales::percent) + 
  # 分面，进一步按group分组，x轴范围自由否则位置异常，swith设置标签底部，并调置去除图例背景
  facet_grid( ~Batch , scales = "free_x", switch = "x") +  
  ...
  # 关闭x轴刻度和标签
  # theme(axis.ticks.x = element_blank(), axis.text.x = element_blank())+
  xlab("Individual Time Series")+ylab("Fungi Microbiome Load (Total Reads per Host DNA) ")+
  theme(axis.text.x = element_text(size=7.5,angle = 90))+
  ...
  # geom_segment(data=data_all$Microbiome_Load,aes(x=Age,y=median(data_all$Microbiome_Load)), colour="purple", size=2)+
  coord_flip()+
  # scale_fill_brewer(palette="Blues")+
  main_theme
p

ggsave("../plot/lifeCycleRice/microbiome_load_timeCourse.pdf", p)


# p = ggplot(data_all[data_all$Batch %in% "NO.2",], aes(x=Other, y = Microbiome_Load,color=Other, fill=Other)) + 
...
# 
# 
# ggsave("../plot/lifeCycleRice/microbiome_load_NO2.pdf", p)
...
idp <- match(row.names(qpcr),rownames(micro_load))
ratio<- as.data.frame(qpcr[idp,1])
bac_reads_AA_cor <- bac_reads_AA*ratio[2:123,1]
...
ggsave("../plot/lifeCycleRice/AA_tax_stack_phylumpro_sample.pdf", p)

# p = ggplot(data_all[data_all$Batch %in% "NO.2",], aes(x=variable, y = value, fill = phylumpro,group=variable)) + 
#   geom_bar(stat = "identity")+ 
#   # geom_bar(stat = "identity",position="fill")+ 
#   # scale_y_continuous(labels = scales::percent) + 
#   # 分面，进一步按group分组，x轴范围自由否则位置异常，swith设置标签底部，并调置去除图例背景
...
#   main_theme
# p
# ggsave("../plot/lifeCycleRice/AA_tax_stack_phylumpro_sample_NO2.pdf", p)


######AA taxa statistics

p <- ggplot(data_all[data_all$Batch %in% "NO.1"&data_all$phylumpro %in% c("Bacteroidetes","Proteobacteria"),], aes(x=Other, y=value,color=Other,full=Other)) +
...
ggsave("../plot/lifeCycleRice/AA_BacteroidetesProteobacteria_NO1_stat.pdf", p)


p <- ggplot(data_all[data_all$Batch %in% "NO.2"&data_all$phylumpro %in% c("Bacteroidetes","Proteobacteria"),], aes(x=Other, y=value,color=Other,full=Other)) +
  geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
  geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
...
p
ggsave("../plot/lifeCycleRice/AA_BacteroidetesProteobacteria_NO2_stat.pdf", p)


p <- ggplot(data_all[data_all$Batch %in% "NO.1"&data_all$phylumpro %in% c("Actinobacteria","Firmicutes"),], aes(x=Other, y=value,color=Other,full=Other)) +
  geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
  geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
...
p
ggsave("../plot/lifeCycleRice/AA_ActinobacteriaFirmicutes_NO1_stat.pdf", p)


p <- ggplot(data_all[data_all$Batch %in% "NO.2"&data_all$phylumpro %in% c("Actinobacteria","Firmicutes"),], aes(x=Other, y=value,color=Other,full=Other)) +
  geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
  geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
  labs(x="Growth Condition_batch_NO2", y="Relative phylum abundance") +
  # geom_signif(comparisons =list(c("E05/5", "E00")),
...
p
ggsave("../plot/lifeCycleRice/AA_ActinobacteriaFirmicutes_NO2_stat.pdf", p)


p <- ggplot(data_all[data_all$Batch %in% "NO.1"&data_all$phylumpro %in% c("Proteobacteria","Low Abundance"),], aes(x=Other, y=value,color=Other,full=Other)) +
  geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
  geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
  labs(x="Growth Condition_batch_NO1", y="Relative phylum abundance") +
  # geom_signif(comparisons =list(c("E05/5", "E00")),
...
p
ggsave("../plot/lifeCycleRice/AA_ProteobacteriaLow_Abundance_NO1_stat.pdf", p)

p <- ggplot(data_all[data_all$Batch %in% "NO.2"&data_all$phylumpro %in% c("Proteobacteria","Low Abundance"),], aes(x=Other, y=value,color=Other,full=Other)) +
  geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
  geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
  labs(x="Growth Condition_batch_NO2", y="Relative phylum abundance") +
  # geom_signif(comparisons =list(c("E05/5", "E00")),
...
p
ggsave("../plot/lifeCycleRice/AA_ProteobacteriaLow_Abundance_NO2_stat.pdf", p, width = 5, height = 3)

p <- ggplot(data_all[data_all$Batch %in% "NO.1"&data_all$phylumpro %in% c("Chloroflexi","Tenericutes"),], aes(x=Other, y=value,color=Other, fill=Other)) +
  geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
  geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
  labs(x="Growth Condition_batch_NO1", y="Relative phylum abundance") +
  # geom_signif(comparisons =list(c("E05/5", "E00")),
...
p
ggsave("../plot/lifeCycleRice/AA_ChloroflexiTenericutes_NO1_stat.pdf", p)

p <- ggplot(data_all[data_all$Batch %in% "NO.2"&data_all$phylumpro %in% c("Chloroflexi","Tenericutes"),], aes(x=Other, y=value,color=Other, fill=Other)) +
  geom_boxplot(alpha=0.51, outlier.size=0, size=0.7, width=0.1) +
  geom_jitter(aes(shape=Other), position=position_jitter(0.17), size=1, alpha=0.7) +
  labs(x="Growth Condition_batch_NO2", y="Relative phylum abundance") +
...
p
ggsave("../plot/lifeCycleRice/AA_ChloroflexiTenericutes_NO2_stat.pdf", p)
...
# write.table(mean_table,file = "../bac_New/table/lifeCycleRice/silva_Facet_Natural_bacteria_RA_mean_box_line.txt",sep = '\t',row.names = F,quote = F)
# 
...
# ggsave("../plot/lifeCycleRice/alpha_shannon_boxplot.pdf", p)
# 
# write.table(df,file = "../table/lifeCycleRice/alpha_shannon.txt",sep = '\t',row.names = F,quote = F)
...
# # write.table(index,file = "../bac_New/table/lifeCycleRice/silva_Facet_Natural_bacteria_AA_alpha.txt",sep = '\t',row.names = F,quote = F)
# 
# 
# 
# #### beta diversity #######
...
write.table(otu_table_for_beta,file = "../table/lifeCycleRice/otu_table_for_beta.txt",sep = '\t',quote = F,row.names = F,col.names = T)
...
#### CSS normalization
# 
# loadMeta("../bac_New/table/lifeCycleRice/otu_table.biom")
# 
...
# }
# 
# CSS("../bac_New/table/lifeCycleRice/otu_table.biom","../bac_New/table/lifeCycleRice/otu_table_css.biom" , 1)
# 
# "CSS" <-function(input_path, out_path, output_CSS_statistics=NULL) {
#   obj = load_biom("../bac_New/table/lifeCycleRice/otu_table.biom")
#   p = cumNormStatFast(obj)
#   obj = cumNorm(obj, p = p)
#   if (!is.null(output_CSS_statistics)) {
#     exportStats(obj, p=p, file = file.path(output_CSS_statistics))
...
bray_curtis <- read.table("../table/lifeCycleRice/beta/bray_curtis_otu_table_for_beta.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# bray_curtis_1 <- read.table("../bac_New/table/lifeCycleRice/beta_div/bray_curtis_otu_table_css.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# eulian <- read.table("../bac_New/table/lifeCycleRice/beta_div/euclidean_otu_table_css.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# 
...
p <- ggplot(points, aes(x=x, y=y, color=Genotype,shape=Batch)) +
...
ggsave("../plot/lifeCycleRice/PCoA_BC_Batch.pdf", p)
...
ggsave("../plot/lifeCycleRice/PCoA_BC_WD.pdf", p)
ggsave("../bac_New/figure/eulcian_PCoA_BC.pdf", p)
write.table(points,file = "../table/lifeCycleRice/WD_Natural_bacteria_betaDiv_points.txt",sep = '\t',row.names = F,quote = F)
...
weighed_unifrac <- read.table("../bac_New/table/lifeCycleRice/beta_div_unifrac/weighted_unifrac_table.from_txt_json.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# bray_curtis_1 <- read.table("../bac_New/table/lifeCycleRice/beta_div/bray_curtis_otu_table_css.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# eulian <- read.table("../bac_New/table/lifeCycleRice/beta_div/euclidean_otu_table_css.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# 
...
un_weighed_unifrac <- read.table("../bac_New/table/lifeCycleRice/beta_div_unifrac/unweighted_unifrac_table.from_txt_json.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# bray_curtis_1 <- read.table("../bac_New/table/lifeCycleRice/beta_div/bray_curtis_otu_table_css.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# eulian <- read.table("../bac_New/table/lifeCycleRice/beta_div/euclidean_otu_table_css.txt", sep="\t", header=T, row.names = 1,check.names=F,stringsAsFactors = F)
# 
...
# write.table(mean_A,file = "../bac_New/table/lifeCycleRice/silva_RA_ratio_B_oneFold_vs_A_BacSpikePertur_line_chart_accuracy.txt",sep = '\t',row.names = F,quote = F)
# 
# ####BC###
...
# write.table(mean_C,file = "../bac_New/table/lifeCycleRice/silva_RA_ratio_B_oneFold_vsC_twoFold_line_chart_accuracy.txt",sep = '\t',row.names = F,quote = F)
# 
# 
# # plot absolute abundance 
# 
...
# 
# micro_load <- as.data.frame(colSums(bac_reads_AA))
# write.table(bac_reads_AA,file = "../table/lifeCycleRice/bac_reads_AA.txt",sep = '\t',row.names = F,quote = F)
# 
# 
# 
# 
...
# # write.table(mean_table,file = "../bac_New/table/lifeCycleRice/silva_Facet_Natural_bacteria_AA_mean_box_line.txt",sep = '\t',row.names = F,quote = F)
# # 
# # ####AA Ratio
# # 
# # 
...
# # 
# # write.table(mean_A,file = "../bac_New/table/lifeCycleRice/silva_modified_Facet_Natural_bacteria_AA_chart_line_AB.txt",sep = '\t',row.names = F,quote = F)
# # 
# # 
# # 
# # ####AC###
...
# # write.table(mean_C,file = "../bac_New/table/lifeCycleRice/silva_modified_Facet_Natural_bacteria_AA_mean_chart_line_AC.txt",sep = '\t',row.names = F,quote = F)
# # 
