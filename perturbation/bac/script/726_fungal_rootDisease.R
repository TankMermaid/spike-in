...
tem <- "natural disturbation"
setwd("~/xiaoxuan/180213/180731_fun/script")
...
figures.dir <- paste("~/xiaoxuan/180213/180731_fun/plot/",tem,'/',sep = '')
table.dir <- paste("~/xiaoxuan/180213/180731_fun/table/",tem,'/',sep = '')
...
write.table(sub_table_nor,file = "~/xiaoxuan/180213/180731_fun/RA/otutab_norm_RA.txt",sep = '\t',row.names = T,quote = F)
...
detailName <- paste("natural_disturbation_fungal_Community_",spike[2],sep = '_')
...
p <- ggplot(index, aes(x=Genotype, y=value, color=Genotype)) +
  ...
  geom_jitter(aes(shape=Description), position=position_jitter(0.17), size=1, alpha=0.7) +
...
qpcr <- read.table("../doc/qPCR_corrected.txt", row.names= 1,header=T, sep="\t",stringsAsFactors = F)
...
# data_all[data_all$Genotype %in% "E03",]

p = ggplot(data_all[!data_all$Description %in% "E00",], aes(x=Genotype, y = Microbiome_Load,color=Genotype, fill=Genotype)) + 
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.4, fill="transparent") +
...
ggsave("../plot/microbiome_load_DiseaseVsHealthy.pdf", p)
...
###AA taxa plot
qpcr <- read.delim("../doc/qPCR_corrected.txt", row.names= 1,header=T, sep="\t",stringsAsFactors = F)
...
write.table(Fun_reads_AA_cor,file = "~/xiaoxuan/180213/180731_fun/table/adjusted_absAbundance.txt",sep = '\t',row.names = T,quote = F)
write.table(Fun_reads_AA_cor,file = "~/xiaoxuan/180213/180731_fun/spike/otutab_norm_AA.txt",sep = '\t',row.names = T,quote = F)
