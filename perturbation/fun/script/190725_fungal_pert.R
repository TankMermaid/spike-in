...
setwd("~/xiaoxuan/180213/180731_fun_pert/script/")
...
figures.dir <- paste("~/xiaoxuan/180213/180731_fun_pert/plot/",tem,'/',sep = '')
table.dir <- paste("~/xiaoxuan/180213/180731_fun_pert/table/",tem,'/',sep = '')
...
design = read.table("../doc/zh11_design.txt", header=T, row.names= 1, sep="\t") 
...
# sub_OTU <- sub_OTU[-pos,]
pos <- grep("OTU_3$",rownames(sub_OTU))
...
sub_table_nor <- as.data.frame(t(sub_table_nor_t))
...
write.table(sub_table_nor,file = "~/xiaoxuan/180213/180731_fun_pert/RA/otutab_norm_RA.txt",sep = '\t',row.names = T,quote = F)
...
detailName <- paste("natureperturbation_fungal_Community_",spike[2],sep = '_')
...
ggsave("../bac_New/figure/perturbation_unoise_bac/silva_alpha_shannon_boxplot.pdf", p)

write.table(df,file = "../bac_New/table/silva_shannon.txt",sep = '\t',row.names = F,quote = F)
...
# absAbundance <- sweep(sub_table_2,2,as.numeric(sub_table_2[idx,-len]),'/')
# 
# ####### reorder ##########
# 
...
write.table(Fun_reads_AA,file = "../table/unadjustedAbsAbundance.xls",sep = '\t',row.names = T)
...
p = ggplot(data_all[!data_all$Description %in% "E00"&!data_all$Genotype %in% "ZH11_Plas_Bac" ,], aes(x=Genotype, y = Microbiome_Load,color=Genotype, fill=Genotype)) + 
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.4, fill="transparent") +
...
Fun_reads_AA_cor <- sweep(Fun_reads_AA_cor, 2, as.vector(qpcr_ratio$spike_host_ratio/3), "*")


# otutab_norm_AA <- read.delim("../spikein/otutab_norm_AA.txt", row.names= 1,header=T, sep="\t",stringsAsFactors = F)
# 
# otutab_norm_AA


colSums(Fun_reads_AA_cor)
write.table(Fun_reads_AA_cor,file = "~/xiaoxuan/180213/180731_fun_pert/table/adjusted_absAbundance.txt",sep = '\t',row.names = T,quote = F)
write.table(Fun_reads_AA_cor,file = "~/xiaoxuan/180213/180731_fun_pert/spikein/fun_pert_otutab_norm_AA.txt",sep = '\t',row.names = T,quote = F)
...
ggsave("../RA/730_fungi_PCoA12_BC.pdf", p)
...
ggsave("../RA/730_fungi_PCoA13_BC.pdf", p)
...
ggsave("../RA/730_fungi_PCoA23_BC.pdf", p)
